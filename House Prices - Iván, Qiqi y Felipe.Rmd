...
Estas dos variables cuentan **los baños totales y los medios baños** de la vivienda. Estas toman el **valor 0** cuando la vivienda **no dispone de esos tipo de baño**. Graficaremos a continuación las dos variables para comprobar como se distribuyen en función de nuestra objetivo `SalePrice`.
...
# Fase 5.1: Evaluación en Validación Cruzada V-Folds (KNN)
...
El modelo consistirá en un **árbol de clasificación** que implementaremos con `decision_tree()` y `mode = "regression"`.
...
# Fase 5.2: Evaluación en Validación Cruzada V-Folds (Árboles)
...
Como se puede observar en las métricas, los **p-valor** coligados a los parámetros $\beta$ de las variables `GarageQual`, `Electrical_SBrkr`, `PavedDrive_Y`, `YrSold_X2010` y `SaleCondition_Partial` son **superiores a 0.05**. Si trabajamos con un nivel de significación habitual del 5 % (0.05), el p-valor es mayor que ese nivel de significación, por lo que **no podemos rechazar la hipótesis nula**, esto es, **la no significación individual** de estas variables al 5 % de significación. Por ende, a la luz de los resultados, `GarageQual`, `Electrical_SBrkr`, `PavedDrive_Y`, `YrSold_X2010` y `SaleCondition_Partial` son variables **no significativas individualmente**. Optaremos por **eliminarlas** posteriormente en la selección de modelos porque solo aportan ruido.
...
Nos quedamos finalmente con la **regresión multivariante con selección de modelos**.
